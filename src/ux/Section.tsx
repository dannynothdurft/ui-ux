...
interface SectionContentProps {
  ...
  horizontalAlign?: 'left' | 'center' | 'right' | 'between' | 'around' | 'evenly';
...
export const SectionContent: React.FC<SectionContentProps> = ({
  ...
  const verticalAlignClass = verticalAlign ? `section-content--valign-${verticalAlign}` : '';
  const horizontalAlignClass = horizontalAlign ? `section-content--halign-${horizontalAlign}` : '';
...
export const SectionImage: React.FC<SectionImageProps> = ({ 
  ...
  const aspectRatioClass = aspectRatio ? `section-image__img--${aspectRatio}` : '';

  return (
    <div className={`section-image ${fullWidthClass} ${objectFitClass} ${className}`}>
      <Image
        src={src}
        alt={alt}
        width={width}
...
export const SectionText: React.FC<SectionTextProps> = ({ 
  ...
  return (
    <div className={`section-text ${alignClass} ${className}`}>
      {children}
    </div>
  );
};